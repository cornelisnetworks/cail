...
/* Scratch byte on host for PCIe read fence in cail_gpu_flush_rdma_writes().
   Pinned memory avoids staging through a pageable bounce buffer. */
static char  *flush_host_byte = NULL;
static char  *flush_dev_byte  = NULL;
...
extern "C" int cail_cuda_init(void) {
    ...
    /* Allocate a pinned host byte and a device byte for the PCIe read fence
       used by cail_gpu_flush_rdma_writes().  cudaMemcpy D→H with a single
       byte forces a PCIe read transaction that flushes all prior posted
       writes (GDRCopy BAR stores, NIC RDMA) to GPU memory. */
    if (cudaHostAlloc(&flush_host_byte, 1, cudaHostAllocDefault) != cudaSuccess)
        flush_host_byte = NULL;
    if (cudaMalloc(&flush_dev_byte, 1) != cudaSuccess)
        flush_dev_byte = NULL;
...
extern "C" void cail_cuda_finalize(void) {
    ...
    if (flush_dev_byte)  { cudaFree(flush_dev_byte);      flush_dev_byte  = NULL; }
    if (cail_stream != 0) {
...
extern "C" int cail_gpu_flush_rdma_writes(void) {
    if (flush_dev_byte && flush_host_byte) {
        cudaMemcpy(flush_host_byte, flush_dev_byte, 1, cudaMemcpyDeviceToHost);
    }
    return 0;
}
